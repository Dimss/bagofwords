...
- The `boot_stack.sh` located at `tools/agent/boot_stack.sh` manage development stack. 
  start the stack by running `tools/agent/boot_stack.sh --dev`
  stop the stack by running `tools/agent/boot_stack.sh --stop`
  check status by running `tools/agent/boot_stack.sh --status`
  form more information about managing development environment read script: `tools/agent/boot_stack.sh`
